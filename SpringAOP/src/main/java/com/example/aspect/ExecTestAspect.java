...
public class ExecTestAspect {
	...
	@AfterReturning(pointcut="execPointCut()",returning="obj")
	...
	@AfterThrowing(pointcut="execPointCut()",throwing="exception")
	public void doAfterThrowing(Throwable exception){
		String methodname=Thread.currentThread().getStackTrace()[1].getMethodName();
		String result="异常信息(若出现异常):"+exception;
		log.error("类名:[{}],方法名:[{}],结果:[{}]", classname,methodname,result);
	}
